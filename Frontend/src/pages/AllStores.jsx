  ...
  return (
    <div className="h-screen bg-background text-textMain font-sans flex flex-col overflow-hidden">
      ...
      <div className="flex-1 relative flex flex-col pt-16">
        ...
        <div className="h-[65%] w-full relative z-0 border-b border-borderCustom/20 overflow-hidden">
           <div className="w-full h-full brightness-[0.9] contrast-110">
              ...
              >
                 <TileLayer url={`https://{s}.basemaps.cartocdn.com/${document.documentElement.classList.contains('dark') ? 'dark_all' : 'light_all'}/{z}/{x}/{y}{r}.png`} />
                      ...
                      icon={new L.DivIcon({
                        ...
                        html: `
                          <div class="relative flex items-center">
                            <div class="w-4 h-4 bg-accent border-2 border-background rounded-full shadow-premium"></div>
                            <div class="ml-3 px-3 py-1 bg-surface/80 backdrop-blur-md border border-borderCustom rounded-full whitespace-nowrap shadow-xl">
                              <span class="text-[9px] font-black uppercase tracking-widest text-textMain">${s.name}</span>
                            </div>
                          </div>
                        `,
                        iconSize: [16, 16],
                    ...
                    >
                       <Popup className="luxe-popup">
                          <div className="w-[280px] bg-surface border border-borderCustom/40 rounded-[2.5rem] overflow-hidden shadow-2xl translate-y-[-10px]">
                             ...
                             <div className="p-5">
                                <h4 className="text-xl font-black uppercase italic tracking-tighter text-textMain">{s.name}</h4>
                                <div className="mt-3 flex items-center justify-between border-t border-borderCustom/20 pt-3">
                                   <span className="text-[9px] font-black uppercase text-subtext/40">Range: 0.8 KM</span>
                                   <button 
                                     onClick={(e) => { e.stopPropagation(); navigate(`/store/${s._id}`); }}
                                     className="px-4 py-1.5 bg-textMain text-background text-[8px] font-black uppercase tracking-widest rounded-full hover:bg-accent transition-all"
                                   >
                                      Enter
                                   </button>
                                </div>
           ...
           <div className="absolute top-10 left-10 z-20">
              <button onClick={() => navigate('/marketplace')} className="flex items-center gap-4 px-6 py-3 bg-surface/40 backdrop-blur-3xl border border-borderCustom/50 shadow-premium rounded-full text-[10px] font-black uppercase tracking-[0.5em] text-accent hover:bg-accent hover:text-white transition-all">
                 <ArrowLeft size={16} /> Registry Hub
              </button>
           </div>

           <div className="absolute top-10 right-10 z-20 flex flex-col gap-4">
              <div className="flex flex-col bg-surface/40 backdrop-blur-3xl border border-borderCustom/50 shadow-premium rounded-full overflow-hidden">
                 <button onClick={() => setMapZoom(prev => Math.min(prev + 1, 18))} className="p-5 hover:text-accent transition-all"><Plus size={20} /></button>
                 <button onClick={() => setMapZoom(prev => Math.max(prev - 1, 3))} className="p-5 hover:text-accent transition-all border-l border-borderCustom/20"><Minus size={20} /></button>
              </div>
              <button 
                onClick={() => { setMapCenter([23.0225, 72.5714]); setMapZoom(13); setSelectedIndex(0); }}
                className="w-16 h-16 bg-accent rounded-full flex items-center justify-center shadow-2xl hover:scale-110 transition-all text-background group"
              >
                 <Locate size={24} />
              </button>
           </div>
        ...
        <div className="h-[35%] bg-background relative z-10 flex flex-col justify-center overflow-hidden">
